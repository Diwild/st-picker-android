...
class MarketDepth:
    ...
    def _parse(self):
        ...
        if isinstance(raw_buy, list):
            self.buy5 = [(float(b[0]), int(b[1])) for b in raw_buy if isinstance(b, (list, tuple)) and len(b) >= 2]
        if isinstance(raw_sell, list):
            self.sell5 = [(float(s[0]), int(s[1])) for s in raw_sell if isinstance(s, (list, tuple)) and len(s) >= 2]
    ...
    def _parse(self):
        for key, target in [('daily', self.daily), ('weekly', self.weekly), ('monthly', self.monthly)]:
            ...
            if isinstance(raw, list):
                target.extend([KlineItem(item) for item in raw if isinstance(item, dict)])
